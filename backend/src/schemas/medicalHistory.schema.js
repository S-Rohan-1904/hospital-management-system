...
const createMedicalHistorySchema = z.object({
  patient: z.string().min(1, 'description cannot be empty')
  .refine(value => value.trim().length > 0, {
      message: 'patient cannot be empty after trimming whitespace',
    }),
  doctor: z.string().min(1, 'description cannot be empty')
  .refine(value => value.trim().length > 0, {
      message: 'doctor cannot be empty after trimming whitespace',
    }),
  hospital: z.string().min(1, 'description cannot be empty')
  .refine(value => value.trim().length > 0, {
      message: 'hospital cannot be empty after trimming whitespace',
    }),
  startDate: z.string().datetime(),
  endDate: z.string().datetime(),
  diagnosis: z.string().min(1, 'description cannot be empty')
  .refine(value => value.trim().length > 0, {
...
const getMedicalHistorySchema = z.object({
    patientId: z.string().min(1, 'description cannot be empty')
    .refine(value => value.trim().length > 0, {
        message: 'description cannot be empty after trimming whitespace',
      }),
    doctorId: z.string().min(1, 'description cannot be empty')
    .refine(value => value.trim().length > 0, {
        message: 'description cannot be empty after trimming whitespace',
      }),
  });
