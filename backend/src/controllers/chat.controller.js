...
const getUserChats = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json(new ApiResponse(200, chats, "Chats have been successfully fetched"));
    } catch (error) {
        return res
            .status(500)
            .json(new ApiResponse(500, {}, "Something went wrong while fetching chats"))
    }
})
...
const createChat = asyncHandler(async (req, res) => {
    ...
    if (role==="doctor" && specialization) {
        ...
        try {
            const hospital = await Hospital.findOne({ doctors: _id }).populate("doctors");
            ...
            for (let index = 0; index < hospital.doctors.length; index++) {
                ...
                if (doctor.specialization.toLowerCase() === specialization.toLowerCase()) {
            ...
            if (groupChatDoctors.length===0) {
                return res
                    .status(404)
                    .json(new ApiResponse(404, {}, "No doctors found with this specialization"));
            }
        ...
        try {
            ...
            return res
                .status(201)
                .json(new ApiResponse(201, { groupChat, receiver}, "Individual chat has been created"))
        } catch (error) {
            return res
                .status(500)
                .json(
...
const getChatUsersBasedOnRole = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, users, "Users have been successfully fetched"))
    } catch (error) {
        return res
            .status(500)
            .json(new ApiResponse(500, {}, error.message || "Something went wrong while fetching users"))
    }
})
